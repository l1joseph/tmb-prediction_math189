...
### 6.3 Cross-Validated Performance

To obtain honest out-of-sample performance estimates, we evaluate the logistic model using stratified 5-fold cross-validation. The cross-validated AUC for the standard logistic model is 0.917 ± 0.014, representing only a modest decline from the in-sample estimate of 0.921 and confirming that the model generalizes well to held-out data (Figure 25). The narrow standard deviation across folds indicates stable performance regardless of the specific train-test split.

We also evaluate an elastic net logistic regression (L1/L2 penalty with class-weight balancing) as a regularized alternative. The elastic net achieves a nearly identical CV AUC of 0.917 ± 0.014, with the LASSO component zeroing out only one feature (Glioblastoma, a cancer type with a small sample size and intermediate TMB-high prevalence). This confirms that the original predictor set is already parsimonious — regularization does not improve discrimination, and nearly all 36 features after dummy encoding carry useful signal. The top-ranked elastic net coefficients mirror the unpenalized model: MSI status (MSS: -1.66) and cancer type indicators dominate, followed by melanoma (+0.94), non-small cell lung cancer (+0.89), and bladder cancer (+0.52).

![Figure 25: Cross-validated AUC comparison across standard logistic and elastic net models.](figures/cv_auc_comparison.png)

### 6.4 Threshold Optimization

The default 0.5 classification threshold produces high specificity (97.7%) but clinically inadequate sensitivity (49.3%) for TMB-high detection. In an immunotherapy triage context where missing an eligible patient carries significant cost, threshold optimization is essential.

Using cross-validated predicted probabilities to avoid optimistic bias, we evaluate two threshold selection criteria. The F1-optimal threshold of 0.299 maximizes the harmonic mean of precision and recall, yielding sensitivity of 69.2%, specificity of 93.2%, and F1 of 0.626. Youden's J statistic (maximizing sensitivity + specificity - 1) identifies a more aggressive threshold of 0.191, pushing sensitivity to 82.7% at the cost of reduced specificity (87.6%) and precision (46.7%).

| Threshold | Sensitivity | Specificity | Precision | F1 | Accuracy |
|-----------|------------|-------------|-----------|------|----------|
| Default (0.50) | 0.493 | 0.977 | 0.741 | 0.592 | 0.921 |
| F1-optimal (0.30) | 0.692 | 0.932 | 0.571 | 0.626 | 0.904 |
| Youden's J (0.19) | 0.827 | 0.876 | 0.467 | 0.597 | 0.870 |

The threshold analysis plot (Figure 26) visualizes the full sensitivity-specificity-precision tradeoff, while the precision-recall curve (Figure 27) shows an average precision of 0.654, substantially above the 0.116 baseline prevalence. The choice of operating point depends on clinical context: in a screening setting, the Youden threshold captures 83% of TMB-high patients at the cost of more false positives, while the F1-optimal threshold offers a balanced compromise.

![Figure 26: Threshold analysis showing sensitivity, specificity, precision, and F1 as a function of classification threshold.](figures/threshold_analysis.png)

![Figure 27: Precision-recall curve with F1-optimal operating point.](figures/precision_recall_curve.png)

### 6.5 Calibration and Goodness of Fit

The Hosmer-Lemeshow test yields a chi-squared statistic of 13.48 (df = 8, p = 0.096), indicating adequate calibration at the 5% significance level. The calibration curve (Figure 28) shows reasonable agreement between predicted probabilities and observed event rates, though the model tends to slightly underestimate TMB-high probability in the highest-risk decile.

![Figure 28: Calibration plot comparing predicted probabilities to observed TMB-high rates.](figures/calibration_plot.png)
